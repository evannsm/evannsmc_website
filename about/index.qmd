...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function setBoriExpanded(isExpanded) {
    if (!boriToggle || !boriMoments || !boriPanel) return;
  ...
  if (boriPanel && boriToggle && boriMoments) {
    setBoriExpanded(false);

    boriToggle.addEventListener('click', function (event) {
      event.stopPropagation();
      setBoriExpanded(boriToggle.getAttribute('aria-expanded') !== 'true');
    });

    boriPanel.addEventListener('click', function (event) {
      if (event.target.closest('.about-feature-moments')) return;
      if (event.target.closest('button') && !event.target.closest('.about-feature-toggle')) return;
      if (boriToggle.getAttribute('aria-expanded') !== 'true') {
