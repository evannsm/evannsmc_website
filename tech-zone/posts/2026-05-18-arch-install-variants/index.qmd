...
**What this variant gives you:** the full single-boot install from [Article 03](../2026-05-17-arch-luks-btrfs-caelestia/), but condensed and without the Caelestia/Hyprland desktop layer. LUKS2 underneath, Btrfs subvolumes on top, zram for everyday swap, a NoCOW Btrfs swap file for hibernation, snapper + grub-btrfs from day one.
...
### Next: layer the Caelestia desktop on top

This tab stops where Article 03 stops in its install layer — you have an encrypted, snapshot-able, hibernation-ready single-boot Arch system, on the console. The desktop is its own piece of work:

::: {.callout-tip}
## → [Caelestia (Hyprland Desktop) — From Bare Arch to a Working Bar](../2026-05-18-caelestia-desktop/)

PipeWire audio, GPU drivers, AUR helper, the Caelestia bundle, KDE apps under Hyprland, SDDM, wallpaper chain, and the Hyprland 0.55 Lua override path. Pick it up from here.
:::

If you want a different desktop (GNOME, Plasma, sway, dwm, …), skip the Caelestia article and install the relevant `gnome` / `plasma` / `sway` group instead — the install layer this tab leaves you with is desktop-agnostic.
