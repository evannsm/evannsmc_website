---
title: "Pacman, Yay, and the AUR: What Every Prompt Actually Means"
...
date: "2026-05-17"
categories: [arch linux, pacman, aur, yay, package management, reference]
...
**`yay` and `paru`** are *AUR helpers* — they wrap that "clone + makepkg + pacman" loop into a single command and add nice quality-of-life features (parallel downloads, dependency resolution across repo + AUR, interactive search). Caelestia's install script defaults to yay; the prompts and behaviors below also apply (with minor cosmetic differences) to paru.
...
# Part 5 — The AUR and how yay wraps it
...
**`yay -S caelestia-meta`** does roughly:

1. Search the AUR for `caelestia-meta`.
2. Fetch its PKGBUILD into `~/.cache/yay/caelestia-meta/`.
...
That's why yay's prompts come **first** (about the build), then pacman's prompts come **last** (about the install).
