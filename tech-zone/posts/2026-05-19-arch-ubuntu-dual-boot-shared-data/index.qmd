...
description: "A dual-boot install that bolts an Ubuntu 24 LTS install alongside the LUKS+Btrfs single-boot Arch from Article 03 — two independently encrypted root partitions, a shared 150 GiB Btrfs data partition both distros mount read-write, and a shared 36 GiB swap partition. Both roots are LUKS2 + Btrfs (`@`, `@home`, `@snapshots`, `@var_log`, `@swap`). Shared partitions are LUKS-encrypted and auto-unlocked at boot via keyfiles stored inside each distro's already-unlocked root — no double passphrase prompt. Each distro hibernates to its own NoCOW Btrfs swap file inside its own encrypted root, so there are no cross-OS resume-image collisions. GRUB on the single shared ESP, with `os-prober` enabled."
...
- **One shared 150 GiB Btrfs data partition** — also LUKS-encrypted, auto-unlocked at boot from both distros via keyfiles. Mounted at `/data` on Arch and `/data` on Ubuntu, owned by your user on both sides.
- **One shared 36 GiB swap partition** — LUKS-encrypted, keyfile-unlocked, used by both distros as everyday overflow swap.
- **Per-OS hibernation** to a NoCOW Btrfs swap file inside each distro's own encrypted root (no cross-OS resume-image collisions).
...
It stops on the console for the Arch side — desktop layer is your choice, exactly like Article 03 ([Caelestia, Article 05](../2026-05-18-caelestia-desktop/) or a custom Hyprland). Ubuntu's GNOME (or whichever flavor you install) comes up out-of-the-box.
:::
...
2. **How do shared encrypted partitions get unlocked from both sides without prompting twice?** A LUKS container can hold up to 8 keyslots. We add the user's passphrase as slot 0 (for recovery from a live USB), then add a 4096-byte keyfile as slot 1 — and store a copy of that keyfile inside each (already-unlocked) root, referenced from each distro's `/etc/crypttab`. Boot order: root unlock by passphrase → root mounts → crypttab reads keyfile → shared partitions unlock silently.
3. **Resume-image collisions.** Two OSes hibernating to the same swap partition would overwrite each other's resume image. We sidestep this by giving each OS its own NoCOW Btrfs swap file inside its own LUKS root — exactly the [Article 03 pattern](../2026-05-17-arch-luks-btrfs-caelestia/#chapter-7-the-hibernation-swap-file), applied twice.
...
The first three steps are identical to the [basic install](../2026-05-01-first-arch-install/) and [Article 03's Part 1](../2026-05-17-arch-luks-btrfs-caelestia/#part-1--bootstrap-same-as-the-basic-install). In short, from an Arch live USB:
...
## Chapter 1: The five-partition layout

This is the central design decision and the only place the dual-boot story really diverges from Article 03 at the block-device level. Five partitions on one drive:

| Partition | Size               | Filesystem                  | Encryption | Mounted on | Purpose                                            |
|-----------|-------------------:|-----------------------------|------------|------------|----------------------------------------------------|
| `p1`      | 1 GiB              | FAT32                       | none       | `/boot`    | Shared UEFI ESP (kernels, initramfs, GRUB)         |
| `p2`      | 36 GiB             | swap (inside LUKS2)         | LUKS2      | `[SWAP]`   | Shared everyday swap, keyfile-unlocked             |
| `p3`      | 150 GiB            | Btrfs (inside LUKS2)        | LUKS2      | `/data`    | Shared data partition, keyfile-unlocked            |
| `p4`      | (remaining ÷ 2)    | Btrfs (inside LUKS2)        | LUKS2      | `/`        | **Arch** root, passphrase-unlocked                 |
| `p5`      | (remaining ÷ 2)    | Btrfs (inside LUKS2)        | LUKS2      | `/`        | **Ubuntu** root, passphrase-unlocked (installer)   |

::: {.callout-note}
## Doing the math for your disk

The user-facing numbers were specified — 36 GiB swap, 150 GiB shared, the rest split 50/50 between Arch and Ubuntu. For a 2 TiB SSD, that's:
...
= 1813 GiB remaining
  ÷ 2
=  906 GiB each for Arch and Ubuntu
```
...
3. `n` → `36G`  → type **Linux swap**       → `/dev/nvme0n1p2`. (The "swap" type is hint-only here — we'll wrap it in LUKS first, which presents as `crypto_LUKS` regardless.)
4. `n` → `150G` → type **Linux filesystem** → `/dev/nvme0n1p3`.
5. `n` → `906G` → type **Linux filesystem** → `/dev/nvme0n1p4` (Arch root; substitute your half-of-remaining number).
6. `n` → rest   → type **Linux filesystem** → `/dev/nvme0n1p5` (Ubuntu root).
7. `w`, then type `yes`.
...
You should see five children of `nvme0n1`, in order, with the sizes you typed.

---

## Chapter 2: Format the three Arch-side LUKS containers

We format three LUKS containers now (p2, p3, p4) and leave p5 for Ubuntu's installer to format later. The reason: Ubuntu's "Custom layout" installer can unlock an existing LUKS2 container, but it's far more reliable to let it format the target partition itself — that way Ubuntu owns the container's metadata, keyslot policy, and `crypttab` UUID, with no inherited state from us.

LUKS2 is the modern default in `cryptsetup`; there is no good reason to force LUKS1 in 2026.

```bash
cryptsetup luksFormat --type luks2 /dev/nvme0n1p2   # swap
cryptsetup luksFormat --type luks2 /dev/nvme0n1p3   # shared data
cryptsetup luksFormat --type luks2 /dev/nvme0n1p4   # Arch root
```
...
1. **Three distinct passphrases.** Maximum recovery-time clarity ("which one was for which?"). All three go in your password manager. The keyfile mechanism below means you only ever *type* the Arch root passphrase at boot anyway, so the extra two aren't a daily burden — they're rescue-mode-only.
2. **The same passphrase on all three.** Less paperwork, slightly worse cryptographic hygiene (a leaked passphrase exposes all three containers). Defensible if you store everything in the same password manager anyway.

I'm using option 1 — a Bitwarden entry per container, named `LUKS — arch root`, `LUKS — swap`, `LUKS — shared`.

**There is no recovery if you forget all three.** Save them somewhere before continuing.
:::

Now unlock all three to their canonical mapper names:

```bash
cryptsetup open /dev/nvme0n1p2 cryptswap
cryptsetup open /dev/nvme0n1p3 cryptshared
cryptsetup open /dev/nvme0n1p4 cryptroot
```

The mapper names matter — they're the contract between this step, the kernel cmdline, `/etc/crypttab`, and `/etc/fstab`. Pick a naming convention and stick with it. I'm using `cryptroot` for the Arch root (matching Article 03), `cryptshared` for the data partition, and `cryptswap` for swap. (Ubuntu's root will become `cryptroot_ubuntu` when we install it later, to avoid the name colliding with Arch's mapper if both are open from a live USB.)

Format the inside of each container:

```bash
mkfs.fat -F32 /dev/nvme0n1p1                # ESP — outside LUKS, must stay unencrypted so GRUB can read it
mkswap -L cryptswap /dev/mapper/cryptswap   # swap inside its LUKS container
mkfs.btrfs -L shared /dev/mapper/cryptshared
mkfs.btrfs -L arch   /dev/mapper/cryptroot
```

`mkswap` against the **mapper**, not the raw partition — otherwise you'd be putting a swap signature on the raw block device and corrupting the LUKS header. Same logic for the two `mkfs.btrfs` calls.
...
::: {.callout-note}
## We don't `swapon` the shared swap partition from inside the live system

Same reasoning as in Article 03 — activating swap from the live USB's kernel makes it harder to clean up at reboot. The shared swap partition will come up via `/etc/fstab` + `/etc/crypttab` on first boot of the installed system, which is the right time.
:::
...
## Chapter 7: Generate the shared keyfile and add it to the two shared LUKS containers

This is the new ground vs. Article 03. We want the two shared partitions (swap and data) to unlock automatically on every boot of either OS — **without** prompting for an extra passphrase — so we use the standard pattern: each LUKS container holds an additional keyfile in one of its keyslots, and a copy of that keyfile lives inside each (already-unlocked, encrypted) root, referenced from `/etc/crypttab`.
...
4. `systemd-cryptsetup-generator` reads `/etc/crypttab`, finds the two keyfile entries, unlocks `cryptshared` and `cryptswap` using the keyfiles on the *now-mounted* root.
5. `/etc/fstab` mounts `/data` and `swapon`'s the shared swap partition.

That's all of step 4 happening *after* the root is mounted, which is the key thing: the keyfiles never have to be in the initramfs (which would require `cryptkey=` and a more complex setup). They live as plain files on the encrypted root, only readable once that root is unlocked.

Generate the keyfiles inside the chroot (4 KiB random bytes is the cryptsetup-recommended size — enough entropy that brute force is hopeless, small enough to read in one block):
...
dd if=/dev/random of=/etc/cryptsetup-keys.d/swap.key bs=4096 count=1 iflag=fullblock
chmod 0400 /etc/cryptsetup-keys.d/swap.key
```

`mode 0400, root:root` — only root can read the keyfiles, and the encrypted root partition is what protects them at rest. If the disk is stolen and the thief doesn't have the Arch root passphrase, the keyfiles are unreadable along with everything else inside `cryptroot`.

Now add each keyfile to its corresponding LUKS container as keyslot 1. (Keyslot 0 already holds your passphrase from Chapter 2.)

```bash
cryptsetup luksAddKey /dev/nvme0n1p3 /etc/cryptsetup-keys.d/shared.key
# Prompts for any existing passphrase. Type the passphrase you used for the shared container in Chapter 2.

cryptsetup luksAddKey /dev/nvme0n1p2 /etc/cryptsetup-keys.d/swap.key
# Same — type the swap container's existing passphrase.
```

Verify the keyslots:

```bash
cryptsetup luksDump /dev/nvme0n1p3 | grep -A2 Keyslot
cryptsetup luksDump /dev/nvme0n1p2 | grep -A2 Keyslot
```

You should see two **Keyslots** listed (0 and 1) on each — keyslot 0 is your passphrase, keyslot 1 is the keyfile. Either will unlock the container.
...
::: {.callout-note}
## We are reusing keyfiles across Arch and Ubuntu

A second copy of `shared.key` and `swap.key` will live on the Ubuntu root after we install Ubuntu in Part 4 — but the actual bytes are identical. We generate the keyfiles *now*, on the Arch root, and copy them to the Ubuntu root later (Chapter 23). The two LUKS containers only know about *one* keyfile each; either OS's copy of the file unlocks them.

If you'd rather give each OS its own distinct keyfile, generate four keyfiles instead of two (Arch's shared.key + swap.key, Ubuntu's shared.key + swap.key), `luksAddKey` each into the relevant container, and put one pair on each root. This costs you two more keyslots (out of 8 available) per container — fine in practice, but no security gain since both OSes already trust each other for shared-partition access.
:::

---

## Chapter 8: `/etc/crypttab` entries for swap and shared

`/etc/crypttab` on the running (post-root-mount) system tells systemd what additional LUKS containers to unlock and how:

```bash
echo "cryptshared UUID=$(blkid -s UUID -o value /dev/nvme0n1p3) /etc/cryptsetup-keys.d/shared.key luks" \
    >> /etc/crypttab
echo "cryptswap   UUID=$(blkid -s UUID -o value /dev/nvme0n1p2) /etc/cryptsetup-keys.d/swap.key   luks" \
    >> /etc/crypttab
```

Two entries, four columns each:

| Column   | Meaning |
|----------|---------|
| `cryptshared` / `cryptswap` | Mapper name to create. Must match what's referenced in `/etc/fstab` (e.g. `/dev/mapper/cryptshared`). |
| `UUID=…`                    | The LUKS partition's UUID — the same string you'd find in `blkid /dev/nvme0n1p3`. Pinning by UUID survives device-renaming and partition reordering. |
| `/etc/cryptsetup-keys.d/*.key` | Path to the keyfile **on the unlocked root** (so it's only readable after the root mounts). |
...
Now update `/etc/fstab` so that mounting `/data` waits for `cryptshared` to be unlocked, and `swapon` waits for `cryptswap`. `genfstab` already wrote the `/data` line in Chapter 5 (Btrfs mount of `/dev/mapper/cryptshared`), but it didn't add the swap line — append it now:
...
```bash
grep -E 'cryptshared|cryptswap' /etc/fstab
```

You should see exactly two lines — one Btrfs `/data` line and one swap line.
...
Identical to [Article 03 Chapter 7](../2026-05-17-arch-luks-btrfs-caelestia/#chapter-7-the-hibernation-swap-file). The shared swap partition (p2, keyfile-unlocked) is *everyday* swap. Hibernation goes to a NoCOW swap file *inside* the Arch root, so resume target is unambiguous and there's no collision with Ubuntu's own hibernation swap file.
...
The explicit `pri=-2` is documentation — zram at priority 100 will be used first, the shared swap partition (priority `defaults`, ~-1) second, the hibernation swap file (priority -2) only under heavy memory pressure or when systemd writes the hibernation image.
...
Note the three-tier swap arrangement we end up with:

| Tier | What                              | Priority | When the kernel uses it                                   |
|------|-----------------------------------|----------|-----------------------------------------------------------|
| 1    | `/dev/zram0`                      | 100      | Almost all everyday swap-out lands here.                  |
| 2    | `/dev/mapper/cryptswap` (shared)  | defaults | Overflow when zram is full.                               |
| 3    | `/swap/swapfile` (hibernation)    | -2       | Only on actual hibernation, or last-resort overflow.      |
...
```bash
echo "cryptroot UUID=$(blkid -s UUID -o value /dev/nvme0n1p4) none luks" \
...
::: {.callout-note}
## Why the shared / swap LUKS entries aren't in `crypttab.initramfs`

The initramfs's job is to unlock just enough to mount `/` — and that's only `cryptroot`. The two shared partitions are unlocked by the *running system's* `systemd-cryptsetup` units, generated from `/etc/crypttab` (the non-`.initramfs` one). They'd fail to unlock in the initramfs anyway because the keyfile path `/etc/cryptsetup-keys.d/shared.key` doesn't exist until `/` is mounted.
:::
...
Get the **Arch root LUKS partition's UUID** (`/dev/nvme0n1p4`, not the mapper):
...
grep '^GRUB_DISABLE_OS_PROBER='        /etc/default/grub
cat /etc/fstab     | grep -E 'subvol=|swap|cryptshared|cryptswap'
...
cryptswap   UUID=<swap-luks-uuid>   /etc/cryptsetup-keys.d/swap.key   luks
-r-------- 1 root root 4096 May 19 14:23 shared.key
-r-------- 1 root root 4096 May 19 14:23 swap.key
```
...
5. systemd starts; `systemd-cryptsetup@cryptshared.service` and `…@cryptswap.service` read their keyfiles and unlock the two shared containers silently.
6. `/data` mounts; `cryptswap` swap activates.
7. Login prompt.
...
Log in as your user. Run the Article 03 sanity block plus the new shared-partition checks:
...
swapon --show                    # zram0 (pri 100), cryptswap (pri -1 or so), /swap/swapfile (pri -2)
cryptsetup status cryptshared    # active LUKS2 mapping
cryptsetup status cryptswap      # active LUKS2 mapping
```

**What pass looks like:** all three `cryptsetup status` outputs print `is active and is in use`, `findmnt /data` shows the shared Btrfs filesystem, and `swapon --show` lists three devices.

Reconnect to Wi-Fi, re-enable NTP, verify timezone — same as [Article 03 Part 3](../2026-05-17-arch-luks-btrfs-caelestia/#part-3--first-boot-snapper-snapshots-sanity-checks).
...
Identical to [Article 03 Part 3](../2026-05-17-arch-luks-btrfs-caelestia/#configure-snapper-for--properly) — won't repeat here. Once those are green:
...
You now have a verified, encrypted, snapshot-able Arch install with two shared LUKS partitions sitting unused-by-Ubuntu-yet. Time to bring up Ubuntu in p5 (the empty partition we left for it in Chapter 1).
...
4. In the partition table editor:
    ...
    - **`/dev/nvme0n1p2`** — leave it alone (Ubuntu will recognize it as a LUKS container; don't let the installer touch it).
    - **`/dev/nvme0n1p3`** — same, leave alone.
    - **`/dev/nvme0n1p4`** — same, leave alone (this is Arch's root; Ubuntu must not even read it).
    - **`/dev/nvme0n1p5`** — select it, **"Change"**, **set "Use as: physical volume for encryption"**, **format checked**. The installer will prompt for a LUKS passphrase. Use a distinct passphrase from Arch's root — save it in your password manager as `LUKS — ubuntu root`.
    - After confirming the encryption setup, the partition will show as a mapper (e.g., `/dev/mapper/nvme0n1p5_crypt`). Select that mapper, **"Change"**, set "Use as: btrfs journaling file system", "Mount point: /", **format checked**.
...
cryptsetup open /dev/nvme0n1p5 nvme0n1p5_crypt
mkfs.btrfs -L ubuntu /dev/mapper/nvme0n1p5_crypt
...
`update-grub` with `os-prober` enabled (which is the Ubuntu default) discovers Arch on `/dev/nvme0n1p4` after unlocking the LUKS container with the passphrase it doesn't have… well, it discovers the LUKS container exists and adds a "Arch Linux GNU/Linux" entry that chainloads our `/boot/EFI/GRUB/grubx64.efi`. Functionally: at the GRUB menu after Ubuntu install, picking "Ubuntu" boots Ubuntu directly; picking "Arch Linux" chains to Arch's GRUB and from there to Arch.
:::
...
```bash
sudo cryptsetup open /dev/nvme0n1p5 ubunturoot
...
The mapper name `ubunturoot` here is a *temporary* name for the live ISO's use — the installed system uses whatever name Ubuntu's installer chose (typically `nvme0n1p5_crypt`). Inside the chroot, you're working with the installed system's filesystem layout.

Inside the chroot, mount the still-unlocked Arch root just long enough to copy the keyfiles:

```bash
cryptsetup open /dev/nvme0n1p4 archroot_tmp
mount -o subvol=@ /dev/mapper/archroot_tmp /mnt
mkdir -p /etc/cryptsetup-keys.d
cp /mnt/etc/cryptsetup-keys.d/shared.key /etc/cryptsetup-keys.d/
cp /mnt/etc/cryptsetup-keys.d/swap.key   /etc/cryptsetup-keys.d/
chmod 700 /etc/cryptsetup-keys.d
...
The keyfiles are now on the *Ubuntu* encrypted root with the same bytes as the *Arch* copy. Either OS's copy unlocks the same LUKS containers.

Now write Ubuntu's `/etc/crypttab` entries (Ubuntu uses the same format as Arch, parsed by the same `systemd-cryptsetup-generator`):

```bash
SHARED_UUID=$(blkid -s UUID -o value /dev/nvme0n1p3)
...
echo "cryptswap   UUID=${SWAP_UUID}   /etc/cryptsetup-keys.d/swap.key   luks" >> /etc/crypttab
```

(The Ubuntu installer already wrote a root-LUKS line at the top of `/etc/crypttab` — leave that line alone, just append these two below it.)

Add the corresponding `/etc/fstab` lines:
...
```bash
# Find the Btrfs filesystem mounted at / — should be /dev/mapper/nvme0n1p5_crypt
findmnt /
...
Edit `/etc/default/grub`. Ubuntu's `GRUB_CMDLINE_LINUX_DEFAULT` typically reads `quiet splash`. Add the `resume=` and `resume_offset=` parameters, using the *Ubuntu root LUKS partition's* UUID (not Arch's), the mapper name Ubuntu uses (`nvme0n1p5_crypt`), and the offset from the previous step:

```bash
sudo blkid -s UUID -o value /dev/nvme0n1p5     # save this as <UBUNTU_LUKS_UUID>
...
```text
GRUB_CMDLINE_LINUX_DEFAULT="quiet splash resume=/dev/mapper/nvme0n1p5_crypt resume_offset=<UBUNTU_RESUME_OFFSET>"
```

Tell Ubuntu's initramfs builder about the hibernation device:

```bash
echo "RESUME=/dev/mapper/nvme0n1p5_crypt" | sudo tee /etc/initramfs-tools/conf.d/resume
...
On Ubuntu, `initramfs-tools` (the analog of Arch's `mkinitcpio`) reads `/etc/initramfs-tools/conf.d/resume` at build time and bakes the resume device into the initramfs so userspace's hibernation tooling agrees with the kernel cmdline. Without this file, `update-initramfs` defaults to detecting the *largest* swap from `/etc/fstab` — which on this system would be the 36 GiB shared swap partition (`cryptswap`), wrong for hibernation. Pinning to the per-OS swap file mapper is what avoids the cross-OS collision.
:::

Confirm Ubuntu sees the shared partitions and swap correctly:

```bash
findmnt /data                 # /dev/mapper/cryptshared, subvol=@data
swapon --show                 # zram (if Ubuntu set it up — recent Ubuntu does), cryptswap, /swap/swapfile
...
```text
Found Ubuntu 24.04 LTS (24.04) on /dev/nvme0n1p5
```
...
```bash
findmnt /                       # subvol=/@, on nvme0n1p5_crypt
findmnt /home                   # subvol=/@home
findmnt /data                   # subvol=/@data, on cryptshared (same UUID as Arch sees)
swapon --show                   # /swap/swapfile (Ubuntu's), cryptswap (shared)
cryptsetup status nvme0n1p5_crypt
cryptsetup status cryptshared
cryptsetup status cryptswap
...
**What pass looks like at the system level:** each OS hibernates and resumes into itself without touching the other, both mount `/data` with identical file ownership, both see exactly three swap devices in priority order, and the GRUB menu lists both OSes regardless of which `grub-mkconfig`/`update-grub` regenerated it last.
...
- The keyfile was never added to the LUKS container's keyslot. `cryptsetup luksDump /dev/nvme0n1p3 | grep -i keyslot` should show **two** active keyslots; if only one, re-run `cryptsetup luksAddKey /dev/nvme0n1p3 /etc/cryptsetup-keys.d/shared.key`.
- The UUID in `/etc/crypttab` doesn't match the partition's actual LUKS UUID. `blkid /dev/nvme0n1p3` — compare.
...
Recover the same way: boot the Arch USB, `cryptsetup open /dev/nvme0n1p4 cryptroot`, `mount -o subvol=@ /dev/mapper/cryptroot /mnt`, `mount /dev/nvme0n1p1 /mnt/boot`, `arch-chroot /mnt`, fix, `mkinitcpio -P`, `grub-mkconfig -o /boot/grub/grub.cfg`, reboot.
...
This means both OSes' `resume=` parameters point at the *same* device, and whichever boots last reads the other's image. The fix: each OS's `resume=` parameter should point at its own per-OS swap file's underlying mapper (`/dev/mapper/cryptroot` for Arch, `/dev/mapper/nvme0n1p5_crypt` for Ubuntu) — NOT the shared `cryptswap` mapper.

If you accidentally set `resume=/dev/mapper/cryptswap` on either OS, fix the cmdline (Arch: `/etc/default/grub` → `grub-mkconfig`; Ubuntu: `/etc/default/grub` → `update-grub`; Ubuntu also: `/etc/initramfs-tools/conf.d/resume` → `update-initramfs -u -k all`).
...
[**Caelestia (Article 05)**](../2026-05-18-caelestia-desktop/) for a turnkey Hyprland bundle, or a custom Hyprland (upcoming). Neither affects Ubuntu — desktop choice lives entirely on the Arch side.
:::
...
If you ever want to bolt Windows onto this layout, that's [Article 10](../../#tz-planned-10) territory — the dual-boot above doesn't preclude it, but Windows adds BitLocker, signed-bootloader, and ESP-conflict considerations that deserve their own article.
:::
