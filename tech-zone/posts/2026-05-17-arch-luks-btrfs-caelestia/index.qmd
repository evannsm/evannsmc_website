...
::: {.callout-tip}
## Setting your first wallpaper (and how Caelestia's wallpaper layer works)

Caelestia doesn't use a separate wallpaper daemon (`swww`, `hyprpaper`, etc.) — its **Quickshell-based bar is also the wallpaper layer**. The same Quickshell process that renders your status bar at the `OVERLAY` Wayland layer also renders your wallpaper at the `BACKGROUND` layer. So there's nothing to install — `pacman -Qi swww` will report "not found" and that's correct.
