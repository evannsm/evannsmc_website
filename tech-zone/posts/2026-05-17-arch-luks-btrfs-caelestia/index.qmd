---
title: "Arch + LUKS + Btrfs + Caelestia"
...
# Part 4 — Layer on the Caelestia desktop (separate article)

You now have a working, encrypted, snapshot-able single-boot Arch system. The PipeWire audio stack, the Caelestia Hyprland bundle, the GPU drivers, the AUR helper, the KDE apps under Hyprland, the SDDM login manager, the wallpaper chain that survives reboots, and the Hyprland 0.55 Lua override path are all covered in their own dedicated article so this one can stay focused on the install layer:

::: {.callout-tip}
## → [Caelestia (Hyprland Desktop) — From Bare Arch to a Working Bar](../2026-05-18-caelestia-desktop/)

The post-install half: audio, GPU, AUR, Caelestia itself, KDE-Frameworks-under-Hyprland apps, SDDM, wallpaper, and customization paths. Pick it up on top of the install you just finished.
:::

Before you click through, a short final-checks block for the install layer itself — these are the commands you can run *right now*, from a TTY login, to confirm everything from Parts 1–3 came up correctly:
...
If those return sane output, the encrypted Btrfs install is rock-solid and you're ready to layer the desktop on top.
