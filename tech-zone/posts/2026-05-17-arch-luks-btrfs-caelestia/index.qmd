...
- **A black desktop background** — Caelestia doesn't set a wallpaper out-of-the-box. The bar, side panels, and Quickshell widgets render correctly *on top of* black; that's the default fallback Hyprland uses until a wallpaper backend (e.g., `swww`, `hyprpaper`, or Caelestia's own `caelestia wallpaper`) takes over the background layer. Not a bug — just an empty wallpaper slot.
