...
You now have a working, encrypted, snapshot-able single-boot Arch system on the console. **The next step — putting a graphical session in front of it — is its own choice**, and I deliberately keep it out of this article so the install layer above stays desktop-agnostic. PipeWire audio, GPU drivers, the AUR helper, the login manager, the bar, the wallpaper layer, every keybind — none of that is required for the install to be considered "done." It's a separate body of work.

Two paths, depending on how much opinionation you want from your desktop:

::: {.callout-tip}
## Path A → Caelestia: a turnkey Hyprland bundle (Article 05)

[**Caelestia (Hyprland Desktop) — From Bare Arch to a Working Bar**](../2026-05-18-caelestia-desktop/) walks through a single coherent dotfile bundle: a Quickshell-based status bar, themed terminal (`foot`), `fish` + `starship`, a curated tool set, the Material-You-style wallpaper-driven retheming, GPU drivers (Intel / AMD / NVIDIA / hybrid), the AUR helper, KDE-Frameworks apps under Hyprland, and SDDM. Fast to get to "looks like a screenshot" — but you inherit the Caelestia project's design opinions wholesale, and on older hardware the Quickshell bar adds measurable latency. See [my honest retrospective at the top of Article 05](../2026-05-18-caelestia-desktop/#caveats-from-actually-using-it) before committing.
:::

::: {.callout-tip}
## Path B → Custom Hyprland (upcoming)

A from-scratch Hyprland setup built piece by piece — your own status bar choice (`waybar`, `ironbar`, `eww`, plain `hyprland-eyepatch`, …), your own terminal, your own keybinds, your own theme — with no project-bundle layer on top. Slower to "first screenshot" but every component is one you picked, and there's no surprise opinion baked in. This is the path I'm taking on my next laptop. The article is dashed in the upcoming section on the [Tech Zone landing](../../).
:::

If you don't know which to pick: try **Caelestia** first if you want a polished Hyprland desktop in an evening and don't mind inheriting opinions you'll later strip; pick **Custom Hyprland** if you'd rather take longer and own every line of `hyprland.conf` / `hyprland.lua` from day one.

Before you click through to either, a short final-checks block for the install layer itself — these are the commands you can run *right now*, from a TTY login, to confirm everything from Parts 1–3 came up correctly:
...
If those return sane output, the encrypted Btrfs install is rock-solid and you're ready to layer the desktop on top — Path A or Path B above, your call.
